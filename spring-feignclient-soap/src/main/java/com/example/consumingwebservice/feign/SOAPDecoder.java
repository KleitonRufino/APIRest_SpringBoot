 ...
 *
 * <p>
 * A SOAP Fault can be returned with a 200 HTTP code. Hence, faults could be
 * handled with no error on the HTTP layer. In this case, you'll certainly have
 * to catch {@link SOAPFaultException} to get fault from your API client
 * service. In the other case (Faults are returned with 4xx or 5xx HTTP error
 * code), you may use {@link SOAPErrorDecoder} in your API configuration.
 *
 * <pre>
 *
...
//			throw new SoapException(MensagemExceptionUtils.SOAP_ERRO);
		...
		try {
			...
			if (message.getSOAPBody() != null) {
				...
				//	throw new SoapException(message.getSOAPBody().getFault().getFaultString());
	...
	public static class Builder {
		...
		/**
		 * Alters the behavior of the code to use the {@link SOAPBody#getFirstChild()}
		 * in place of {@link SOAPBody#extractContentAsDocument()}.
		 *
